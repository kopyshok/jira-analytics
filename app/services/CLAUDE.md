...
- `config.py` — настройки раздела одной записью `AppSetting['team_desk_config']`: группы статусов (`dev` / `waiting` / `todo` / `done`), статусы очереди работы, шесть порогов подсветки, типы подзадач, типы задач «по исполнителю» и `developer_roles` (роли из реестра, попадающие в срез; по умолчанию `["dev"]` — аналитики/РП/консультанты отсекаются в эндпоинте). Статус вне групп → `unassigned`, расчёты не ломает. Статусная модель Jira меняется — поэтому настройка, не константа.
