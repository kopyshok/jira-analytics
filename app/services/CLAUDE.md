...
Занятость сотрудников в планах других команд (чистое чтение). Опорный план команды на квартал (`reference_plans`): утверждённый сценарий → `is_baseline` → «Готово» → свежесть (`_plan_sort_key`); нет утверждённого — план свежайшего черновика, `provisional=True`; форки и планы без сценария не участвуют. `external_bookings(team=...)` — брони в опорных планах всех команд, кроме `team` (`team=None` — всех): квартала и прошлого квартала (его хвосты заходят в этот на месяц запаса), по окну `start`–`end`; пять запросов на любой объём; фаза без посуточной раскладки размазывается по будням. У брони `is_borrowing` (команда брони взяла человека не из своего состава — по кварталу её плана). `subtractable(bookings, borrowed)` — правило «сначала домашняя команда»: привлечённому вычитаются все брони, своему — только команд, где он тоже состоит; им пользуются расчёт плана, расшифровка фазы и признак устаревания диаграммы (`stale_teams`). План при расчёте запоминает `external_fingerprint` — хеши вычитаемых броней по командам (`fingerprint`); `stale_teams` сравнивает с ним текущие вычитаемые брони: устарели команды, чьи брони появились, пропали или сдвинулись (пересчёт чужого плана с той же раскладкой план не старит; отпечатка нет — устарел, если вычитаемые брони есть). Закреплённая дата начала раскладывается общим `_allocate_hours_with_breakdown` по этой же доступности. `subtract_occupancy` вычитается из доступности в `compute_schedule`: по ней идут раскладка и сдвиги/переназначения выравнивателя (`placement_availability`), а перегрузку выравниватель меряет по «сырой» ёмкости. `overlap_days` — живой конфликт `CROSS_TEAM_OVERLAP` (`_live_conflicts` в эндпоинтах: диаграмма, список конфликтов, обе расшифровки; окно — квартал + месяц запаса; не хранится, id с префиксом `live:`, PATCH статуса → 409). «Привлечённый» — не состоял в команде плана ни дня квартала (`borrowed_ids`). `quarter_load_pct` — загрузка за квартал для выбора исполнителя. Брони вычитает и `ResourceBaseService` (база и «На бэклог» сценария; за день — не больше остатка после обязательных работ). «Разработчик» из Jira — [jira_developer.py](jira_developer.py): активный с ролью разработчика или без роли; запасной подсчёт по поддереву — только незакрытые задачи. `_assign_employees` берёт его, только если ёмкости квартала хватает на часы разработки; фаза, разложенная не на все часы, даёт конфликт `UNPLACED_HOURS`. `ResourceBaseService` вычитает только брони без `is_borrowing`, остальные показывает справочно (`borrowed_by_other_teams_by_role`). Кандидаты в исполнители фазы и строки сценария — общая [assignee_candidates.py](assignee_candidates.py); исполнитель строки сценария встаёт на фазу своей роли (`_scenario_executor`), из чужой команды — только выбранный вручную (`BacklogItem.assignee_manual`, обновление из Jira его не затирает до смены исполнителя в Jira — `backlog_service.apply_jira_assignee`).
...
## team_desk ([team_desk/](team_desk/))

Рабочий стол тимлида — контроль задач в разрезе разработчиков. Четыре модуля:

- `config.py` — настройки раздела одной записью `AppSetting['team_desk_config']`: группы статусов (`dev` / `waiting` / `todo` / `done`), статусы очереди работы, `wip_statuses` (по умолчанию `В РАБОТЕ` — только по ним считается `in_progress` и виджет «Задач в работе одновременно»; группа `dev` шире и для лимита не годится), `hidden_statuses` (по умолчанию `Backlog` — задача не показывается вообще), шесть порогов подсветки, типы подзадач, типы задач «по исполнителю» и `developer_roles` (роли из реестра, попадающие в срез; по умолчанию `["dev"]` — аналитики/РП/консультанты отсекаются в эндпоинте). Статус вне групп → `unassigned`, расчёты не ломает. Статусная модель Jira меняется — поэтому настройка, не константа.
- `flags.py` — чистые функции: `compute_flags` (десять признаков, включая `idlespent` — часы списаны, а задача в группе «не начаты», `orphan` — подзадача, родителя которой в срезе нет, и `alien` — в задачу списался другой разработчик) и `flag_signature` (снимок причины). ORM сюда не заходит.
- `query.py` — `build_overview(developer_ids, only_open, show_reviewed, show_done_subtasks, period_start, period_end)`: выборка задач (поле «Разработчик» + тех. анализ по исполнителю), факт из `Worklog`, признаки, сводка на человека с медианой точности оценок. В сводке есть `status_counts` — разбивка по конкретным статусам по тому же составу, что и `total_issues` (их сумма обязана совпадать); отдаётся целиком, отбор показываемых статусов — на фронте. Окно периода вместе с `only_open` даёт третий режим среза: открытые сейчас ∪ созданные в окне (`jira_created_at`) ∪ закрытые в окне (`resolved_at`). `show_done_subtasks` (по умолчанию `True`) вторым запросом добирает закрытые подзадачи под уже выбранными родителями — иначе родитель с полностью закрытой декомпозицией выглядит неразбитым. Добор ограничен теми же `developer_ids`: иначе в сводке всплывёт карточка постороннего человека с нулём задач.
- `workload.py` — `queue_for_developers`: очередь работы в часах и рабочих днях на окно вперёд, минус дни отсутствий.

**Инвариант «факт — часы владельца».** `fact_hours` задачи — часы того, кто
указан «Разработчиком» (у тех. анализа — исполнителя), списанные в саму задачу и
в её подзадачи. Часы других ролей (qa, analyst, без роли) в разделе не участвуют
вовсе: раздел про работу программистов, и оценка в задаче тоже только на неё.
Часы другого разработчика идут в `alien_hours` + признак `alien`, но не в факт —
работу двигает владелец. `fact_by_person` — разбивка для подсказки, только
разработчики.

**Инвариант «подзадача — это декомпозиция».** Оценка подзадачи повторяет часть
родительской, поэтому в счётчики, суммы оценок и очередь идут только
самостоятельные задачи (`is_standalone` в строке среза): не подзадача либо
подзадача, родителя которой в срезе нет. Факт наоборот роллапится вверх — часы
ВЛАДЕЛЬЦА из всех подзадач (включая закрытые и отфильтрованные) приплюсовываются
к родителю, потому что разработчик списывает и в основную задачу, и в подчинённые. Очередь
считает остаток `оценка − факт` (не меньше нуля), поэтому задача в работе тоже
даёт вклад. Подзадача без родителя в срезе — ошибка данных: признак `orphan`,
считается как самостоятельная, чтобы часы не потерялись.
- `marks.py` — отметки «просмотрено». **Ключевой инвариант:** отметка живёт, пока `signature` совпадает с текущей причиной; разошлась — запись удаляется, признак возвращается. Иначе отметка навсегда прятала бы реальную проблему.

## stale_field_cleanup ([stale_field_cleanup.py](stale_field_cleanup.py))

`clear_stale_developer_field(db, jira, dry_run=False)` — чистка «мёртвых»
значений поля «Разработчик». Jira при переносе задачи в другой проект сохраняет
ранее заполненные кастомные поля, даже если в новом проекте их нет на экранах:
значение приезжает по API и задача продолжает числиться за разработчиком (стол
тимлида, KPI, аналитика).

**Решение принимается по проекту, а не по связке «проект + тип».** Отдельный тип
задач может не показывать поле на экране редактирования, хотя значение живое —
так в проекте разработки устроены заявки поддержки (950 задач в тенанте, часы
списывает именно указанный разработчик). Проект «живой», если поле доступно хотя
бы на одном из его типов; чистим только проекты, где поля нет нигде.

Доступность — `JiraClient.get_editable_field_ids(issue_key)`; опрашивается до
`PROBE_LIMIT` карточек на связку, перебор типов проекта прерывается на первой
живой. Сбой опроса (нет прав, сеть) **не** считается ответом: иначе обрыв связи
затирал бы данные. Стадия конвейера — `StaleDeveloperFieldStage` (non-critical,
в режимах normal и full после задач). Разовый прогон по накопленному —
`scripts/clear_stale_developer_field.py` (без `--apply` только показывает).

## kpi ([kpi/](kpi/))

Расчёт КЭ. Период задаётся парой «последний месяц» + `months` (длина): месяц,
квартал, произвольный отрезок — единого «режима» нет, см. `period_bounds` /
`months_in_period` / `previous_period` / `period_quarter` в
[kpi_service.py](kpi/kpi_service.py).

- **Итог периода — одна дробь**, а не среднее месячных (`compute_employee_period`).
  Помесячные значения считаются отдельно и кладутся в `months_breakdown` строки.
- **Норматив Cycle Time** живёт на квартал; период через несколько кварталов
  берёт среднее по тем, где норматив заведён (`_norm_for`).
- **Утверждение — целый квартал** (`build_approval_payload` / `save_approval`,
  ключ `team+year+quarter`). Снимок подставляется в отчёт только при точном
  совпадении периода с подписанным кварталом (`_approvals_for_period`); тренд
  всегда считается вживую и лишь помечает точки признаком `approved`.
- Выключатель `approval_enabled` — в `KpiSettings` (AppSetting
  `kpi_approval_enabled`, по умолчанию выключен).
