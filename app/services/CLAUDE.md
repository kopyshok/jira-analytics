...
- `flags.py` — чистые функции: `compute_flags` (девять признаков, включая `idlespent` — часы списаны, а задача в группе «не начаты», и `orphan` — подзадача, родителя которой в срезе нет) и `flag_signature` (снимок причины). ORM сюда не заходит.
...
ВСЕХ подзадач (включая закрытые и отфильтрованные) приплюсовываются к родителю,
потому что разработчик списывает и в основную задачу, и в подчинённые. Очередь
