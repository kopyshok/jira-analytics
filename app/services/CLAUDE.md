...
- `config.py` — настройки раздела одной записью `AppSetting['team_desk_config']`: группы статусов (`dev` / `waiting` / `todo` / `done`), статусы очереди работы, шесть порогов подсветки, типы подзадач и типы задач «по исполнителю». Статус вне групп → `unassigned`, расчёты не ломает. Статусная модель Jira меняется — поэтому настройка, не константа.
