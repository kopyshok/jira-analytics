...
- result_flow_blocks: массив 3-5 объектов {label, status}. Это интеграционный/процессный flow проекта. status ∈ ["source", "flow", "done"]. Первый — обычно "source", последний — "done" ТОЛЬКО если соответствующая часть проекта реально завершена (большинство связанных дочерних задач в статус-категории "done"). Если работа идёт — "flow". Не ставь "done" по умолчанию.
- result_checklist: массив 3-5 объектов {label, done}. Чек-лист достижений. ВАЖНО: `done: true` ставь ТОЛЬКО если соответствующая работа реально завершена. Опирайся на статусы дочерних задач: ✓ рядом со статусом = задача в статус-категории "done" (закрыто/выполнено). Если задача в работе или открыта — `done: false`. Не ставь true просто потому что задача описана в эпике.
...
def build_prompt(epic_data: dict[str, Any], db: Optional[Session] = None) -> str:
    ...
    if summaries:
        ...
        for s in summaries:
            ...
            parts.append(f"\n— {s['key']} [{status}{done_mark}] — {s['summary']}")
            if s.get("goal_text"):
