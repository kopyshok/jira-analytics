...
class ProjectsService:
    ...
    def _project_roots_stmt(
        ...
        *,
        ...
        ``team_filter`` полностью применяется внутри запроса только в ветке
        с кварталом (через PlanningScenario.team, см. выше). Без квартала
        сюда он не попадает вовсе — list_projects накладывает его сам,
        постфактум, по авторам списаний уже обойдённого поддерева.
