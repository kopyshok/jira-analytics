...
def _plan_sort_key(p) -> tuple:
    ...
    Идентификатор — последняя ступень сравнения: у двух планов квартала обе
    отметки времени могут совпасть (копия, созданная в ту же секунду), и тогда
    победитель определялся порядком строк из БД — то есть менялся от запроса к
    запросу, а вместе с ним прыгали часы в сводке портфеля.
    """
    return (
