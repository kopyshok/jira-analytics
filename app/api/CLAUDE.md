...
| `/users` | `users.py` | self-service: GET/PUT /me/period, /me/analytics-columns, /me/teams, /me/team-desk-filter (шапка стола тимлида целиком: команды, добранные разработчики, режим среза `open\|period\|all`, окно периода, оба переключателя, выбор статусов-счётчиков (пусто = все статусы среза); хранилище — свободный JSON в `users.team_desk_filter`, миграции на новые поля не нужны) |
